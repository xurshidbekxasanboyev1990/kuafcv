...
import { AlertCircle, Award, CheckCircle, Clock, Download, File, Plus, X, XCircle } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useEffect, useRef, useState } from 'react';

const CATEGORY = 'AWARDS';
const CATEGORY_LABEL = 'Mukofotlar va yutuqlar';
const CATEGORY_ICON = '🏆';

export default function AwardsPortfolioPage() {
  ...
  return (
    <MainLayout showMarquee={false}>
      <div className="p-4 md:p-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-red-500 mt-2">Olimpiada natijalari, tanlovlar, stipendiyalar, rasmiy e'tiroflar</p>
          </div>
        ...
        {loadingData ? (
          ...
          <div className="text-center py-12 bg-white rounded-xl border border-red-100">
            <Award className="mx-auto text-red-300 mb-4" size={64} />
            <h3 className="text-lg font-medium text-red-800 mb-2">Hozircha mukofotlar yo'q</h3>
            <p className="text-red-600 mb-4">Birinchi mukofotingizni qo'shing</p>
      ...
      {showModal && (
        <AwardsPortfolioModal
          onClose={() => setShowModal(false)}
          onSuccess={() => {
            setShowModal(false);
            fetchPortfolios();
            setMessage({ type: 'success', text: 'Mukofot qo\'shildi' });
...
function AwardsPortfolioModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 md:p-8 max-h-[90vh] overflow-y-auto transform transition-all animate-slideUp">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-800">Mukofot va Yutuqlar</h2>
              <p className="text-sm text-gray-500 mt-0.5">Olimpiada va tanlovlar</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value.slice(0, 200) })} required maxLength={200} placeholder="Masalan: Respublika olimpiadasi I o'rin" className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-800 placeholder-gray-400 transition-all hover:border-gray-300" aria-label="Mukofot nomi" />
            <p className="text-xs text-gray-500">Tanlov yoki olimpiada nomini kiriting</p>
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
              Tavsif
            ...
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value.slice(0, 1000) })} rows={4} maxLength={1000} placeholder="O'rin, fan/yo'nalish, ishtirokchilar soni, erishgan yutuqlar..." className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-800 placeholder-gray-400 resize-none transition-all hover:border-gray-300" aria-label="Mukofot tavsifi" />
            <p className="text-xs text-gray-500">O'rin, fan va erishgan natijalarni kiriting</p>
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
            ...
            <div className="relative">
              <input
                ...
                aria-label="Diplom yoki sertifikatlar tanlash"
              />
              {files.length > 0 && (
            ...
            <p className="text-xs text-gray-500">Diplom yoki sertifikat (maks. 3 ta, har biri 50MB)</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">Masalan: olimpiada, matematika, I o'rin</p>
          </div>
          <div className="flex gap-3 pt-6 border-t border-gray-100">
            ...
            <button type="submit" disabled={loading || !formData.title.trim()} className="flex-1 py-3 px-6 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl hover:from-red-700 hover:to-red-800 font-medium shadow-lg hover:shadow-xl disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all active:scale-95 flex items-center justify-center gap-2" aria-label={loading ? 'Saqlanmoqda' : "Qo'shish"}>
              {loading ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>Saqlanmoqda...</>) : (<><Award size={18} />Qo'shish</>)}
            </button>
          </div>
        </form>
      </div>
