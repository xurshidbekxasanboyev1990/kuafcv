...
const CATEGORY_LABEL = 'Raqamli va texnik tajriba';
const CATEGORY_ICON = '💻';
...
export default function TechnicalPortfolioPage() {
  ...
  return (
    <MainLayout showMarquee={false}>
      <div className="p-4 md:p-8">
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-red-500 mt-2">Dasturlash tajribasi, IT sertifikatlar, texnik ko'nikmalar, kod loyihalar</p>
          </div>
        ...
        {loadingData ? (
          ...
          <div className="text-center py-12 bg-white rounded-xl border border-red-100">
            <Code className="mx-auto text-red-300 mb-4" size={64} />
            <h3 className="text-lg font-medium text-red-800 mb-2">Hozircha texnik tajribalar yo'q</h3>
            <p className="text-red-600 mb-4">Birinchi texnik tajribangizni qo'shing</p>
      ...
      {showModal && (
        <TechnicalPortfolioModal
          onClose={() => setShowModal(false)}
          onSuccess={() => {
            setShowModal(false);
            fetchPortfolios();
            setMessage({ type: 'success', text: 'Texnik tajriba qo\'shildi' });
...
function TechnicalPortfolioModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const addTag = () => {
    ...
    if (formData.tags.includes(trimmedTag)) { setError('Teg allaqachon mavjud'); return; }
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 md:p-8 max-h-[90vh] overflow-y-auto transform transition-all animate-slideUp">
        <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-gray-800">Texnik Tajriba</h2>
              <p className="text-sm text-gray-500 mt-0.5">Sertifikat va ko'nikmalar</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-2">
            ...
            <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value.slice(0, 200) })} required maxLength={200} placeholder="Masalan: Python dasturlash sertifikati" className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-800 placeholder-gray-400 transition-all hover:border-gray-300" aria-label="Sertifikat yoki ko'nikma nomi" />
            <p className="text-xs text-gray-500">Texnik ko'nikma yoki sertifikat nomini kiriting</p>
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
              Tavsif
            ...
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value.slice(0, 1000) })} rows={4} maxLength={1000} placeholder="Texnologiya, daraja, ishlatilgan loyihalar..." className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 text-gray-800 placeholder-gray-400 resize-none transition-all hover:border-gray-300" aria-label="Tavsif" />
            <p className="text-xs text-gray-500">Ko'nikmangiz darajasi va tajribangizni kiriting</p>
          </div>
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-gray-700 flex items-center gap-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-500">Masalan: Python, JavaScript, Docker</p>
          </div>
          <div className="flex gap-3 pt-6 border-t border-gray-100">
            ...
            <button type="submit" disabled={loading || !formData.title.trim()} className="flex-1 py-3 px-6 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-xl hover:from-red-700 hover:to-red-800 font-medium shadow-lg hover:shadow-xl disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed transition-all active:scale-95 flex items-center justify-center gap-2" aria-label={loading ? 'Saqlanmoqda' : "Qo'shish"}>
              {loading ? (<><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>Saqlanmoqda...</>) : (<><Code size={18} />Qo'shish</>)}
            </button>
          </div>
        </form>
      </div>
